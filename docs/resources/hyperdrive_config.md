...
- `mtls` (Attributes) The mTLS configuration for connecting to the origin database. When this block is omitted, the remote mTLS configuration is left unmanaged: it is tracked in state and preserved on updates. Set `mtls = {}` to remove the remote mTLS configuration. (see [below for nested schema](#nestedatt--mtls))
