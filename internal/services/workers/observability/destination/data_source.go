...
func (d *destinationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	iter := d.client.Workers.Observability.Destinations.ListAutoPaging(ctx, params)
	for iter.Next() {
		...
		data.Type = types.StringValue(string(destination.Configuration.Type))
		data.URL = types.StringValue(destination.Configuration.URL)
		data.LogpushDataset = types.StringValue(string(destination.Configuration.LogpushDataset))
